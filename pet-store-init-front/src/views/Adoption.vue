...
export default {
  ...
  methods: {
    ...
    listMyAdopted: function (){
      let address = this.$cookies.get("address");
      this.axios.get(`myAdoption/listAdoptedPet?address=${address}`).then((response)=>{
        if(response.data.code === 200){
          console.log("*********")
          let getRes = response.data.result
          for (let i = 0;i<getRes.length;i++){
            if(getRes[i]>0){
              this.adoptionList.push(this.defaultList[i])
            }
          }
        }
      })
